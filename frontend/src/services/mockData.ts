  ...
  private generateResources(count: number = 50): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (provider === 'AWS') {
        ...
        const serviceTypes = RESOURCE_TYPES.AWS[service] || RESOURCE_TYPES.AWS['EC2'];
        ...
        const serviceTypes = RESOURCE_TYPES.Azure[service] || RESOURCE_TYPES.Azure['Virtual Machines'];
        resourceType = serviceTypes[Math.floor(random() * serviceTypes.length)];
      } else {
        region = GCP_REGIONS[Math.floor(random() * GCP_REGIONS.length)];
        service = GCP_SERVICES[Math.floor(random() * GCP_SERVICES.length)];
        
        const serviceTypes = RESOURCE_TYPES.GCP[service] || RESOURCE_TYPES.GCP['Compute Engine'];
        resourceType = serviceTypes[Math.floor(random() * serviceTypes.length)];
      }
